...
public class WXAudioModule extends WXModule implements IWXAudio, Destroyable {
    ...
    public void load(Map<String, String> options, @Nullable JSCallback statusCallback) {
        ...
        // step2 create player and save variable in map
        collectPlayerIdByInstanceId(mWXSDKInstance.getInstanceId(), id);
        if(optionsMap.containsKey(id)) {
            autoStartPlayWhenReady.remove(id);
            loopWhenPlayEnded.remove(id);
        }else {
            optionsMap.put(id, options);
        }
        callbackMap.put(id, statusCallback);
        changeStatus(id, IWXAudio.MEDIA_STATUS_INIT);
        MediaPlayer player = getPlayer(id); // here create MediaPlay instance

        // step3 process option
        if(null != options.get(IWXAudio.KEY_VOLUME)) {
            Float volume = 0F;
            try {
                volume = Float.parseFloat(options.get(IWXAudio.KEY_VOLUME));
        ...
        if(String.valueOf(Boolean.TRUE).equals(options.get(IWXAudio.KEY_AUTOPLAY))) {
            autoStartPlayWhenReady.add(id);
        }
        if(String.valueOf(Boolean.TRUE).equals(options.get(IWXAudio.KEY_LOOP))) {
            loopWhenPlayEnded.add(id);
        }
    ...
    private void invokeCallback(Map<String, Object> callback) {
        if(null == callback.get(IWXAudio.KEY_ID)) {
            return;
        }
        Long id = (Long) callback.get(IWXAudio.KEY_ID);
    ...
    private void collectPlayerIdByInstanceId(String instanceId, long id) {
    ...
    public void destroy() {
        ...
        instancePlayerIdMap.remove(mWXSDKInstance.getInstanceId());
    }
}
