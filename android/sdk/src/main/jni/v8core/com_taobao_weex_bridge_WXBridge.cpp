...
jbyteArray char_array2jbyteArray(JNIEnv *env, v8::String::Utf8Value &origin);
...
jint Java_com_taobao_weex_bridge_WXBridge_initFramework(JNIEnv *env,
    ...
    jmethodID m_InfoCollect = env->GetMethodID(c_params, "getShouldInfoCollect", "()Ljava/lang/String;");
    jobject ifInfoCollect = env->CallObjectMethod(params, m_InfoCollect);

    env->DeleteLocalRef(c_params);

    //if(ifInfoCollect==false)close infocollect
    if(ifInfoCollect!=NULL){
        const char *c_str = env->GetStringUTFChars((jstring)ifInfoCollect, NULL);
        if(c_str!=NULL&&0==strcmp("false",c_str)){
            LOGA("ifInfoCollect==false");
            env->ReleaseStringUTFChars((jstring)ifInfoCollect, c_str);
            env->DeleteLocalRef(ifInfoCollect);
            return true;
        }
        env->ReleaseStringUTFChars((jstring)ifInfoCollect, c_str);
        env->DeleteLocalRef(ifInfoCollect);
    ...
    const char *c_strappName = env->GetStringUTFChars((jstring)appName, NULL);
    if(c_strappName!= NULL){
        http_request_t request_t = {0};
        const char * path ="http://gm.mmstat.com/weex.1003?appname=";
        int len = strlen(path)+strlen(c_strappName)+1;

        char url[len];
        strncpy(url,"",sizeof(url));
        strncat(url,path,strlen(path));
        strncat(url,c_strappName,strlen(c_strappName));
        request_t.url = url;
        http_result_t *result_t = new http_result_t();
        send_request(&request_t,&result_t);
    }

    env->ReleaseStringUTFChars((jstring)appName, c_strappName);
...
v8::Handle<v8::Value> callNative(const v8::Arguments &args) {
    ...
    if (args[1]->IsObject()) {
        ...
        jTaskString = char_array2jbyteArray(env, str);
        LOGD(" callNative is object");
    } else if (args[1]->IsString()) {
        LOGD("args[1] is String");
        v8::String::Utf8Value tasks(args[1]);
        jTaskString = char_array2jbyteArray(env, tasks);
    }
    ...
    (env)->CallVoidMethod(jThis, tempMethodId, jInstanceId, jTaskString, jCallback);
...
v8::Handle<v8::Value> nativeLog(const v8::Arguments &args) {
    ...
    //从classpath路径下搜索WXLogUtils这个类，并返回该类的Class对象
...
jbyteArray char_array2jbyteArray(JNIEnv *env, v8::String::Utf8Value &origin){
    const char* c_char_array=ToCString(origin);
    size_t length=strlen(c_char_array);
    jbyteArray array = env->NewByteArray (length);
    env->SetByteArrayRegion (array, 0, length, (jbyte *)c_char_array);
    return array;
}